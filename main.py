...
if "last_page" not in st.session_state:
    st.session_state.last_page = send_request()
...
if st.session_state.current_page > st.session_state.last_page:
    st.session_state.current_page = st.session_state.last_page
...
def fetch_data(products ,last_page):
...
with st.sidebar :
    ...
    with col3:
        st.button("Next",on_click=go_next, disabled=st.session_state.current_page == st.session_state.last_page )
       
    page_input = st.number_input(
        "Go to page:",
        min_value=1,
        max_value= st.session_state.last_page,
